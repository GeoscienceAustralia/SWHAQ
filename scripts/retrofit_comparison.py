...
damage_state_change_by_SA1.to_csv("damage_state_change_by_SA1_table.csv")
...
structural_loss_mean_change_by_SA1.to_csv("structural_loss_mean_change_by_SA1_table.csv")
...
#pivot table of damage state by age (don't think this is needed as dw incorperates age)
...
damage_state_change_by_age.to_csv("damage_state_change_by_age_table.csv")

#pivot table of damage state by dw
...
damage_state_change_by_dw.to_csv("damage_state_change_by_dw_table.csv")
...
fig.savefig(pjoin(data_path,
                  f"change_in_DS_by_vuln_func_5%".format(fmt)), dpi=res)
...
plt.savefig(pjoin(data_path,
                  f"change_in_DS_by_vuln_func_bins_5%".format(fmt)), dpi=res)
...
ax.set_ylabel("Number")
fig.savefig(pjoin(data_path,
                  f"change_in_DS_by_vuln_func_5%_2".format(fmt)), dpi=res)
...
plt.legend(title='change in damage state')
plt.savefig(pjoin(data_path,
                  f"change_in_DS_by_vuln_func_bins_5%_2".format(fmt)), dpi=res)
...
ax.set_ylabel("Number")
fig.savefig(pjoin(data_path,
                  f"change_in_DS_by_vuln_func_5%_3".format(fmt)), dpi=res)
...
plt.legend(title='change in damage state')
plt.savefig(pjoin(data_path,
                  f"change_in_DS_by_vuln_func_bins_5%_3".format(fmt)), dpi=res)
...
ax[2,2].set_xlabel('')
fig.savefig(pjoin(data_path,
                  f"old_buildings_retrofit_wind_DS_count_5%".format(fmt)), dpi=res)

def vuln_func_groupby(df,col_name,orig_variable,new_variable):
    df_1 = df.loc[((df[col_name] == orig_variable))]
    df_2 = df_1.loc[((df_1['retro_WIND_VULNERABILITY_FUNCTION_ID'] == new_variable))]
    #how to name the new dataframe based on the strings
    dw354_454_DS_wind = df_2.groupby(['retro_WIND_VULNERABILITY_FUNCTION_ID','wind_bins','dif_DS']).size().reset_index().pivot(columns='dif_DS', index=['retro_WIND_VULNERABILITY_FUNCTION_ID','wind_bins'], values=0)
...
fig.savefig(pjoin(data_path,
                  f"new_buildings_retrofit_wind_DS_count_5%_1".format(fmt)), dpi=res)
...
ax[3].set_xlabel('Wind speed m/s')
fig.savefig(pjoin(data_path,
                  f"new_buildings_retrofit_wind_DS_count_5%_2".format(fmt)), dpi=res)