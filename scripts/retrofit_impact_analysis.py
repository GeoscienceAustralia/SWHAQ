"""
retrofit_impact_analysis.py - simple categorisation of lower/median/upper
damage index values, based on multiple retrofit simulations for a single
TC event (scenario)
...
df.pivot_table(index='LGA_NAME', columns='Median damage state', aggfunc='size',
               fill_value=0).to_csv(os.path.join(
                inputPath, f"{eventid}_med_damage_state.csv"))

df.pivot_table(index='LGA_NAME', columns='Lower damage state', aggfunc='size',
               fill_value=0).to_csv(os.path.join(
                inputPath, f"{eventid}_lower_damage_state.csv"))

df.pivot_table(index='LGA_NAME', columns='Upper damage state', aggfunc='size',
               fill_value=0).to_csv(os.path.join(
                inputPath, f"{eventid}_upper_damage_state.csv"))
